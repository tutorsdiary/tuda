...
public class LoopingCirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCentered) {
            final float circleSize = (mRadius * 2) + (getStrokeWidth() * 2);
            final float width = count * circleSize + ((count - 1) * mExtraSpacing);
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f) - (width / 2.0f) - ((getRealCount() * mRadius) / 4.0f);
        }
    ...
    private int measureLong(int measureSpec) {
        ...
        if ((specMode == MeasureSpec.EXACTLY) || (mViewPager == null)) {
            ...
            final int count = getRealCount();
            result = (int) (getPaddingLeft() + getPaddingRight() + (count * 2 * getStrokeWidth())
